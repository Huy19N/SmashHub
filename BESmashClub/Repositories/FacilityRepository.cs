...
public class FacilityRepository : GenericRepository<Facility>
{
    ...
    public async Task<List<Facility>> GetByOwnerIdAsync(Guid ownerId)
    {
        return await _context.Facilities
            .Include(f => f.Owner)
            .Include(f => f.Courts)
            .Where(f => f.OwnerId == ownerId && (f.IsDelete == null || f.IsDelete == false))
    ...
    public async Task<List<Facility>> GetAllWithDetailsAsync()
    {
        return await _context.Facilities
            .Include(f => f.Owner)
            .Include(f => f.Courts)
            .Where(f => f.IsDelete == null || f.IsDelete == false)
            .OrderByDescending(f => f.CreatedAt)
            .ToListAsync();
    }
    ...
    public async Task<Facility?> GetDetailAsync(int facilityId)
    {
        return await _context.Facilities
            ...
            .FirstOrDefaultAsync(f => f.FacilityId == facilityId && (f.IsDelete == null || f.IsDelete == false));
    }
