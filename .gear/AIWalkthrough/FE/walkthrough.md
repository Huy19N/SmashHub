...
| Update `AdminSettings.jsx`     | Added an interface for administrators to view and configure global system parameters, including `PlatformFeePercentage`. The value can be edited and is persisted through `AdminController` APIs. |
| **Verification & Testing**   | Verified that the backend successfully builds (`dotnet build`), fixed build warnings, and built the frontend UI via `npm run build`. |
