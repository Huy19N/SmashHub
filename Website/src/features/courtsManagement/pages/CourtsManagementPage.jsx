  ...
  return (
    <div className={`min-h-screen ${theme === 'dark' ? 'bg-[#0c0f17]' : 'bg-gray-50'} flex relative overflow-hidden`}>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pl-1 text-xs text-gray-500 dark:text-gray-400 font-label">
                      ...
                      {facilities.length > 0 && (
                        <div className="flex items-center bg-gray-150 dark:bg-white/5 border border-gray-200 dark:border-white/10 p-1 rounded-2xl gap-1 shrink-0 self-start sm:self-auto">
                          <button
                            onClick={() => setActiveTab('list')}
                            className={`px-4 py-2 rounded-xl text-xs font-bold font-label transition-all duration-200 active:scale-95 hover:scale-[1.03] cursor-pointer ${activeTab === 'list'
                          ...
                          {activeFacility?.statusId !== 1 && (
                            <button
                              ...
                              className={`px-4 py-2 rounded-xl text-xs font-bold font-label transition-all duration-200 active:scale-95 hover:scale-[1.03] cursor-pointer ${activeTab === 'add-court'
                                ? 'bg-emerald-600 dark:bg-primary text-white dark:text-[#052e14] shadow-sm'
                                : 'text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'
                                }`}
                            >
                            ...
                            <div className="space-y-4">
                              ...
                              <div className="flex items-start justify-between gap-2">
                                ...
                                <div className="flex flex-col items-end gap-1.5 shrink-0">
                                  {/* Court Status Badge */}
                                  <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[9px] font-bold tracking-wider uppercase border ${!isReady
                                  ...
                                  <span className={`text-[8px] font-bold tracking-wider uppercase ${court.isActive ? 'text-emerald-500' : 'text-gray-400'
                                    }`}>
      ...
      <EditCourtModal
        ...
        fetchFacilityHours={fetchFacilityHours}
        fetchCourtCosts={fetchCourtCosts}
        updateCourtAll={updateCourtAll}
        updateCourt={updateCourt}
        onSave={() => {
          setEditingCourt(null);
          refetchCourts(selectedFacilityId);
        }}
      />
